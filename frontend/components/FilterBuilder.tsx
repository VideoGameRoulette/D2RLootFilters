...
const TABS = [
  { id: "normal", label: "Normal Inferior", shortLabel: "Norm Inf", accentClass: "text-d2-normal" },
...
export function FilterBuilder({ mobile = false }: FilterBuilderProps) {
  ...
  const applyFilterToState = useCallback((filter: LootFilter) => {
    ...
    for (const rule of filter.rules) {
      ...
      if (rar?.length && eqCat?.length && !eq?.length) { for (const cat of eqCat) { const codes = GAME_CATEGORY_BASE_CODES[cat]; if (!codes) continue; const isInferior = rar.includes("lowQuality"); if ((rar.includes("normal") || isInferior) && !rule.filterEtherealSocketed) normal.push(...codes); if ((rar.includes("normal") || isInferior) && rule.filterEtherealSocketed) normal.push(...codes); if (rar.includes("hiQuality") && rule.filterEtherealSocketed) socketedEtherealSuperior.push(...codes); if (rar.includes("magic")) magic.push(...codes); if (rar.includes("rare")) rare.push(...codes); if (rar.includes("unique")) unique.push(...codes); if (rar.includes("set")) set.push(...codes); } }
      const isInferiorRule = rar?.includes("lowQuality");
      if ((rar?.includes("normal") || isInferiorRule) && eq) normal.push(...eq); else if (rule.filterEtherealSocketed && eq && !isInferiorRule && (!rar?.length || rar.includes("normal"))) socketedEthereal.push(...eq); else if (rar?.includes("hiQuality") && rule.filterEtherealSocketed && eq) socketedEtherealSuperior.push(...eq); else if (rar?.includes("magic") && eq) magic.push(...eq); else if (rar?.includes("rare") && eq) rare.push(...eq); else if (rar?.includes("unique") && eq) unique.push(...eq); else if (rar?.includes("set") && eq) set.push(...eq); else if (ic?.length) { const allRunes = ic.every((c) => runeCodeSet.has(c)); if (allRunes) runes.push(...ic); else { for (const c of ic) { if (gemCodeSet.has(c)) gems.push(c); else if (potionCodeSet.has(c)) potions.push(c); else if (endgameCodeSet.has(c)) endgame.push(c); else if (questCodeSet.has(c)) quest.push(c); else miscOther.push(c); } } }
      if (rule.goldFilterValue != null && rule.ruleType === "hide") { goldEnabled = true; goldThreshold = rule.goldFilterValue; }
    }
    setProfileName(filter.name); setSelectedNormalBaseCodes(new Set(normal)); setSelectedSocketedEtherealBaseCodes(new Set(socketedEthereal)); setSelectedSocketedEtherealSuperiorBaseCodes(new Set(socketedEtherealSuperior)); setSelectedMagicBaseCodes(new Set(magic)); setSelectedRareBaseCodes(new Set(rare)); setSelectedUniqueCodes(new Set(unique)); setSelectedSetCodes(new Set(set)); setSelectedRuneCodes(new Set(runes)); setSelectedGemCodes(new Set(gems)); setSelectedPotionCodes(new Set(potions)); setSelectedQuestCodes(new Set(quest)); setSelectedEndgameCodes(new Set(endgame)); setSelectedMiscOtherCodes(new Set(miscOther)); setGoldFilterEnabled(goldEnabled); setGoldFilterThreshold(goldThreshold);
  ...
  const totalSelected = selectedSetCodes.size + selectedUniqueCodes.size + selectedRuneCodes.size + selectedNormalBaseCodes.size + selectedSocketedEtherealBaseCodes.size + selectedSocketedEtherealSuperiorBaseCodes.size + selectedMagicBaseCodes.size + selectedRareBaseCodes.size + selectedGemCodes.size + selectedPotionCodes.size + selectedQuestCodes.size + selectedEndgameCodes.size + selectedMiscOtherCodes.size;
  const canExport = (totalSelected > 0 || goldFilterEnabled) && ruleCount <= MAX_RULES;

  const selectedSummary = useMemo(() => [
    { label: "Norm Inf", count: selectedNormalBaseCodes.size },
  ...
  ], [selectedNormalBaseCodes.size, selectedSocketedEtherealBaseCodes.size, selectedSocketedEtherealSuperiorBaseCodes.size, selectedMagicBaseCodes.size, selectedRareBaseCodes.size, selectedUniqueCodes.size, selectedSetCodes.size, selectedRuneCodes.size, selectedGemCodes.size, selectedPotionCodes.size, selectedQuestCodes.size, selectedEndgameCodes.size, selectedMiscOtherCodes.size]);

  const tabsConfig = useMemo(() => [
    { id: "normal", label: "Normal Inferior", count: baseItemsNormalFiltered.length, selectedCount: selectedNormalBaseCodes.size, accentClass: "text-d2-normal" },
  ...
  ], [baseItemsNormalFiltered.length, baseItemsFiltered.length, baseItemsRareFiltered.length, uniqueItemsFiltered.length, setItemsFiltered.length, runeItemsFiltered.length, gemItemsFiltered.length, potionItemsFiltered.length, questItemsFiltered.length, endgameItemsFiltered.length, miscOtherItemsFiltered.length, selectedNormalBaseCodes.size, selectedSocketedEtherealBaseCodes.size, selectedSocketedEtherealSuperiorBaseCodes.size, selectedMagicBaseCodes.size, selectedRareBaseCodes.size, selectedUniqueCodes.size, selectedSetCodes.size, selectedRuneCodes.size, selectedGemCodes.size, selectedPotionCodes.size, selectedQuestCodes.size, selectedEndgameCodes.size, selectedMiscOtherCodes.size, goldFilterEnabled]);
  ...
  const renderCatalogPanel = () => {
    ...
    if (activeTab === "normal") return <CatalogSection key="normal" title="Normal Inferior" items={baseItemsNormalFiltered} selectedCodes={selectedNormalBaseCodes} onToggle={toggleNormalBase} onSelectAll={selectAllNormalBases} onClearAll={clearAllNormalBases} onPaste={pasteNormalBases} accentClass="text-d2-normal" itemColorClass="text-d2-normal" noContainer fillPanel showMaxSockets itemImageBasePath={base} />;
          ...
          <header className="flex-shrink-0 border-b border-zinc-700/60 bg-zinc-900/90 z-40">
            <div className="px-4 py-3">
              <div className="flex items-center gap-2 gap-y-2 flex-wrap">
                ...
                <input type="text" value={profileName} onChange={(e) => setProfileName(e.target.value)} placeholder="MyFilter" maxLength={14} className="flex-1 min-w-0 bg-zinc-800 border border-zinc-600 rounded-lg px-3 py-2 text-sm font-medium text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-violet-500" />
          ...
          <header className="flex-shrink-0 border-b border-zinc-700/60 bg-zinc-900/70">
            <div className="px-4 md:px-6 lg:px-8 py-3">
              <div className="flex flex-wrap items-center gap-4 gap-y-3">
                ...
                <label className="flex items-center gap-2"><span className="text-zinc-500 text-sm">Profile name</span><input type="text" value={profileName} onChange={(e) => setProfileName(e.target.value)} placeholder="MyFilter (no spaces)" maxLength={14} className="bg-zinc-800/80 border border-zinc-600 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent max-w-[200px]" /></label>
